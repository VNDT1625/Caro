/**
 * RankChangeAnimationV2 Component
 * 
 * Animation khi lên/xuống rank với sub-ranks
 */
...
interface RankChangeAnimationV2Props {
  oldRank: MainRank
  oldTier: RankTier
  oldLevel: RankLevel
  newRank: MainRank
...
export default function RankChangeAnimationV2({
  oldRank,
  oldTier,
  oldLevel,
  newRank,
...
}: RankChangeAnimationV2Props) {
  const { t, language } = useLanguage()
  const [phase, setPhase] = useState<'enter' | 'show' | 'exit'>('enter')

  const comparison = compareRanks(
    { rank: newRank, tier: newTier, level: newLevel },
    { rank: oldRank, tier: oldTier, level: oldLevel }
  )
  const isRankUp = comparison > 0

  const oldRankDisplay = MAIN_RANK_DISPLAY[oldRank]
  const newRankDisplay = MAIN_RANK_DISPLAY[newRank]
  const oldTierDisplay = TIER_DISPLAY[oldTier]
  const newTierDisplay = TIER_DISPLAY[newTier]

  const oldRankIndex = RANK_ORDER.indexOf(oldRank)
  const newRankIndex = RANK_ORDER.indexOf(newRank)
  const isOldHighPhase = oldRankIndex > 2 || oldTier === 'vuot_cap'
  const isNewHighPhase = newRankIndex > 2 || newTier === 'vuot_cap'

  const formatRank = (
    rd: typeof oldRankDisplay, 
    td: typeof oldTierDisplay, 
    level: RankLevel, 
    isHigh: boolean
  ) => {
    ...
    return `${rName} - ${tName} ${level}`
  }

  useEffect(() => {
    const enterTimer = setTimeout(() => setPhase('show'), 100)
    const dismissTimer = setTimeout(() => setPhase('exit'), autoDismissDelay)
    const completeTimer = setTimeout(onComplete, autoDismissDelay + 500)
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{
            ...
            {formatRank(oldRankDisplay, oldTierDisplay, oldLevel, isOldHighPhase)}
          </span>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            {formatRank(newRankDisplay, newTierDisplay, newLevel, isNewHighPhase)}
          </div>
        </div>
